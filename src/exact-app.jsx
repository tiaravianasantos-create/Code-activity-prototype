...
function NetworkGraphic({ kind }) {
  ...
  return <svg className="result-graphic network-graphic" viewBox="0 0 310 230" role="img" aria-label={isSubtour ? "Disconnected route visualization" : "Delivery network visualization"}>
    ...
    <main className="app">
      ...
      <div className="workspace">
        ...
        <section className="work-area code-panel-transition">
          ...
          {status === "result" && <div className="footer-actions">{step < lessons.length - 1 ? <Button onClick={submit}>Submit<ArrowRight size={13}/></Button> : <Button onClick={submit}>Complete<Check size={13}/></Button>}</div>}
        </section>
      </div>
